...
- Mermaid 专业样式：按系统架构、流程、时序、数据图等场景自动套用更清爽的设计主题
- 自动识别粘贴进来的裸 Mermaid 图表源码，并在格式化/宽容渲染时补成可渲染代码块
- 长箭头链路：把 `A → B(子步骤) → C` 这类难读流程整理为多行 `diagram` 文本结构
...
5. Mermaid 图表能按内容自动选择架构/流程/时序/数据等样式
6. 长箭头链路能格式化为多行 `diagram` 代码块
7. 无语言围栏中的文本框线图能格式化为 `diagram` 代码块
8. 搜索、格式化、复制、导出按钮可用
